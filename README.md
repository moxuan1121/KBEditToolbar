...
全选后复制、全选后删除会等待选区更新后再执行第二个动作。WebKit 输入框通过其标准 `selectAll:`/`copy:` 动作处理全选复制，左右移动和移动到首尾使用 WebKit 编辑命令；粘贴通过当前键盘输入通道插入。网页全选清空会额外等待 0.12 秒，再通过 `UIKeyboardImpl deleteFromInput` 删除选区，与 DockX 的动作链一致。
