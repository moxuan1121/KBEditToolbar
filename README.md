...
焊死版长按识别时间为 0.3 秒；识别成长按后不会再触发该按钮的短按动作。

四个工具按钮从最左侧到最右侧组成一块固定的水平触摸板，按钮之间的空隙也可开始或继续滑动：向左滑删除光标左侧直到文本开头的全部内容；向右滑删除光标右侧直到文本末尾的全部内容。为避免误触底部 Home Bar，滑动必须从触摸板顶部 28 pt 区域开始，水平移动至少 40 pt、垂直偏移不超过 12 pt，并满足明显的水平方向意图。手势确认时触发一次轻触反馈；普通点击、长按以及触摸板范围外的地球键和麦克风不受影响。本功能焊死在固定版中，不提供设置项。
