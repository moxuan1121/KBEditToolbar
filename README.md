...
中间的向左、向右两个按钮及它们之间的空隙组成一块固定的水平触摸板；粘贴和收起按钮不再识别滑动，只保留点击与长按。向左滑删除光标左侧直到文本开头的全部内容；向右滑删除光标右侧直到文本末尾的全部内容。为避免误触底部 Home Bar，滑动必须从触摸板顶部 28 pt 区域开始，水平移动至少 40 pt、垂直偏移不超过 12 pt，并满足明显的水平方向意图。手势确认时触发一次轻触反馈；地球键和麦克风不受影响。本功能焊死在固定版中，不提供设置项。
